...
class Trainer:
    ...
    def train(self):
        ...
        for i in tqdm_iterator:
            ...
            self.loss = IOA_Loss(output, gt).to(self.device)
            ...
            tqdm_iterator.set_postfix(loss=f'{self.loss.item():.5f}', refresh=True)
            ...
            if i%self.vis_interval == 0:
                ...
                with torch.no_grad():  # Disable gradient computation
                    for j,eval_image in enumerate(eval_dataloader):  # Assuming eval_image is directly usable
                        ...
                        eval_loss += IOA_Loss(eval_output, eval_gt).item()
                        eval_steps += 1
                ...
                print(f"Average eval loss: {avg_eval_loss:.5f}")
